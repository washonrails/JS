...
const getGithubProfile = async (user, showDataUser) => {
	const url = `https://api.github.com/users/${user}`
	const response = await fetch(url)
	showDataUser(await response.json())
}

const showDataUser = ( {login, bio, followers, following, location} ) => {
	console.log(login)
	console.log(bio)
...
getGithubProfile('washonrails', showDataUser)
...
const numbersDivided = numbers.filter(number => number % 2 === 0 || number % 3 === 0)
...
/*
  ...
  - Exemplos de nomes na "língua do 'P'":
    ...
    - Rafaela => "PRaPfaPePla".
*/
const syllableName = ['wal', 'la', 'ce']
const name = syllableName.map(s => 'P'.concat(s))
...
const occuredReturned = (arr, value) => arr.filter(number => number === value ).length

console.log(occuredReturned(scores, 85))
