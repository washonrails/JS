...
const _map = (arr, mult) => {
  const isNotArray = typeof(arr) !== 'object'

  if (isNotArray || isNaN(mult)) return "Voce deve passar um objeto enumeravel como um Array"


  const arrayMoficado = new Array()
  arr.forEach(number => arrayMoficado.push(number * mult))

  return arrayMoficado
}

console.log(_map(numbers, 2))
console.log(_map(numbers, 3))
console.log(numbers)
...
const x = 'x'
const anotherX = () => {const x = 'y'; return x}
...
const convertToHex = (colorName) => {
  ...
  return colors[colorName]
    ? `O hexadecimal para a cor ${colorName} e ${colors[colorName]}`
    : `Nao temos o hexadecimal equivalente para ${colorName}`
}
